...
const AuthLock = ({ socket, onAuthenticated, onAnimationComplete }) => {
    ...
    useEffect(() => {
        ...
        const handleAuthStatus = (data) => {
            ...
            if (data.authenticated && !isUnlocking) {
                ...
                // Breve pausa de feedback y luego cerrar overlay en el padre (evita capa fija + blur en Safari).
                setTimeout(() => {
    ...
    return (
        ...
        <div
            className={`fixed inset-0 z-[9999] bg-black flex flex-col items-center justify-center font-mono select-none transition-opacity duration-500 ease-out ${isUnlocking ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
        >
            ...
            <div className={`relative flex flex-col items-center gap-6 p-10 border ${borderColor}/30 rounded-lg bg-black/80 backdrop-blur-xl ${shadowColor} transition-all duration-[1500ms]`}>
                ...
                {!isUnlocking && typeof window !== 'undefined' && typeof window.require !== 'function' && (
                    <p className="text-[10px] text-white/45 text-center max-w-xs leading-relaxed">
                        Desde el navegador del móvil, el reconocimiento usa la cámara del equipo donde corre N.E.N.O. Sitúate frente a esa cámara para desbloquear.
                    </p>
                )}
            </div>
